"""Long-horizon comparison between PseudoReality v3 and v3.1.

The comparison keeps v3 frozen and runs the same scenario specifications through
both v3 and v3.1. It checks whether v3.1 introduces possible long-horizon
medium-path persistence / recovery without directly designing a collapse flag or
a forced short-vs-medium cycle.
"""
...
from pseudo_reality.distribution_terrain_v3_1_scenarios import (
    run_default_scenario_suite as run_v31_default_scenario_suite,
    run_scenario_suite as run_v31_scenario_suite,
    run_stable_scenario_suite as run_v31_stable_scenario_suite,
)
...
V31_EXTRA_METRICS = (
    "final_stress_load_distribution_weighted_mean",
    "final_stress_drain_capacity_distribution_weighted_mean",
    "final_productive_stress_distribution_weighted_mean",
...
def _v31_typical_pattern_scenario_suite(*, seed: int = 0, steps: int = 30) -> tuple[pd.DataFrame, dict[str, dict[str, pd.DataFrame]]]:
    return run_v31_scenario_suite(typical_pattern_scenario_specs(seed=seed, steps=steps))
...
def _suite_runners() -> tuple[
    ...
    return (
        ("default", run_v3_default_scenario_suite, run_v31_default_scenario_suite),
        ("stable", run_v3_stable_scenario_suite, run_v31_stable_scenario_suite),
        ("typical", run_v3_typical_pattern_scenario_suite, _v31_typical_pattern_scenario_suite),
    )
...
def _prepare_summary(summary: pd.DataFrame, *, model: str, suite: str, steps: int) -> pd.DataFrame:
    ...
    table["v3_1_long_horizon_readout"] = table.apply(_model_readout, axis=1)
...
def _model_readout(row: pd.Series) -> str:
    ...
    if residual >= 0.35 and damage >= 0.60:
        return "residual_stress_damage_accumulation"
...
def run_v3_1_long_horizon_comparison(
    *,
    seed: int = LONG_HORIZON_COMPARISON_SEED,
    steps_set: tuple[int, ...] = LONG_HORIZON_COMPARISON_STEPS,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Run v3/v3.1 long-horizon comparison tables."""

    model_tables: list[pd.DataFrame] = []
    for steps in steps_set:
        for suite_name, v3_runner, v31_runner in _suite_runners():
            v3_summary, _v3_traces = v3_runner(seed=seed, steps=steps)
            v31_summary, _v31_traces = v31_runner(seed=seed, steps=steps)
            model_tables.append(_prepare_summary(v3_summary, model="v3", suite=suite_name, steps=steps))
            model_tables.append(_prepare_summary(v31_summary, model="v3.1", suite=suite_name, steps=steps))
    by_model = pd.concat(model_tables, ignore_index=True)
    delta = build_v3_1_delta_summary(by_model)
    return by_model, delta


def build_v3_1_delta_summary(by_model: pd.DataFrame) -> pd.DataFrame:
    ...
    for keys, group in by_model.groupby(group_keys, sort=True):
        ...
        if set(group["model"]) != {"v3", "v3.1"}:
            continue
        v3_row = group[group["model"] == "v3"].iloc[0]
        v31_row = group[group["model"] == "v3.1"].iloc[0]
        row: dict[str, object] = {
            "suite": suite,
            "scenario": scenario,
            "validation_steps": int(validation_steps),
            "v3_final_dominance_regime": str(v3_row["final_dominance_regime"]),
            "v31_final_dominance_regime": str(v31_row["final_dominance_regime"]),
        }
        for metric in COMPARISON_METRICS:
            row[f"v3_{metric}"] = float(v3_row[metric])
            row[f"v31_{metric}"] = float(v31_row[metric])
            row[f"{metric}_v31_minus_v3"] = float(v31_row[metric] - v3_row[metric])
        for metric in V31_EXTRA_METRICS:
            if metric in v31_row.index:
                row[f"v31_{metric}"] = float(v31_row[metric])
        row["total_gain_delta_v31_minus_v3"] = row[
            "final_total_gain_distribution_weighted_mean_v31_minus_v3"
        ]
        row["damage_delta_v31_minus_v3"] = row["final_damage_distribution_weighted_mean_v31_minus_v3"]
        row["flow_delta_v31_minus_v3"] = row["final_total_flow_v31_minus_v3"]
        row["v3_1_comparison_readout"] = _comparison_readout(row)
        rows.append(row)
...
def _comparison_readout(row: dict[str, object]) -> str:
    ...
    v31_regime = str(row["v31_final_dominance_regime"])
    total_gain_delta = _numeric_row_value(row, "total_gain_delta_v31_minus_v3")
    ...
    if residual >= 0.35 and damage_delta >= 0.05:
        return "v31_residual_stress_cost_visible"
    if total_gain_delta < -0.05 and damage_delta > 0.05:
        return "v31_worse_on_gain_and_damage"
    if flow_delta > 0.02:
        return "v31_flow_higher"
    return "v31_mixed_comparison"


def export_v3_1_long_horizon_comparison(
    output_dir: str | Path,
    *,
    seed: int = LONG_HORIZON_COMPARISON_SEED,
    steps_set: tuple[int, ...] = LONG_HORIZON_COMPARISON_STEPS,
    ...
    by_model, delta = run_v3_1_long_horizon_comparison(seed=seed, steps_set=steps_set)
    by_model.to_csv(root / "v3_1_long_horizon_by_model_summary.csv", index=False)
    by_model.to_json(root / "v3_1_long_horizon_by_model_summary.json", orient="records", indent=2)
    delta.to_csv(root / "v3_1_long_horizon_delta_summary.csv", index=False)
    delta.to_json(root / "v3_1_long_horizon_delta_summary.json", orient="records", indent=2)
    return by_model, delta
...
if __name__ == "__main__":
    by_model_table, delta_table = export_v3_1_long_horizon_comparison(
        "outputs/pseudoreality-v3-validation/v3-1-long-horizon-comparison",
        seed=LONG_HORIZON_COMPARISON_SEED,
        steps_set=LONG_HORIZON_COMPARISON_STEPS,
    )
    print("\n=== v3/v3.1 by-model readout ===")
