...
DEFAULT_CONTRACT = (
    ROOT
    / "configs"
    / "relation_field_prediction_p2_precursor_audit_contract_v1_1.json"
)
...
def build_relation_field_prediction_p2_precursor_audit(
    case_manifest_path: str | Path,
    output: str | Path,
    *,
    contract_path: str | Path = DEFAULT_CONTRACT,
) -> Path:
    return build_precursor_audit(
        case_manifest_path,
        output,
...
def validate_relation_field_prediction_p2_precursor_audit(
    audit_dir: str | Path,
    case_manifest_path: str | Path,
    *,
    contract_path: str | Path = DEFAULT_CONTRACT,
) -> dict[str, Any]:
    return validate_precursor_audit(
        audit_dir,
        case_manifest_path,
        contract=load_contract(contract_path),
    )
