...
このモジュールは、Task 1〜4で使用した旧方式を再現・監査するために隔離する。
新しいH11意味射影の正本ではなく、将来実装の起点にも直接使用しない。
"""
...
def _calibrated_features(
    ...
    for index, (entry, record) in enumerate(zip(registry["features"], records, strict=True)):
        ...
        value = float(
            np.clip(
                value,
                float(calibration["clip_lower"][index]),
                float(calibration["clip_upper"][index]),
            )
        )
...
def construct_h11_legacy_rc1(
    ...
    for axis_id in evidence_map["axis_order"]:
        ...
        component_weights = (
            {construction["component"]: 1.0}
            if construction["type"] == "base_component"
            else {str(key): float(value) for key, value in construction["components"].items()}
        )
        raw = 0.0
        all_components = True
        ...
        available = bool(
            ...
            and history_frame_count >= minimum_history
            and all_components
            and (coverage >= float(axis["limited_coverage_min"]))
        )
        if available:
            ...
            if denominator > 0.0:
                weighted_confidence = sum(
                    feature_weights.get(feature, 0.0) * float(record_by_id[feature]["confidence"])
                    for feature in available_refs
                ) / denominator
            else:
        ...
        axes[axis_id] = {
            ...
            "transport_value_is_neutral_placeholder": placeholder,
            "available": available,
