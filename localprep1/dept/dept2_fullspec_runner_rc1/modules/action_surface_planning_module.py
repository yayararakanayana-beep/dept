...
class ActionSurfacePlanningModule:
    ...
    def plan(
        ...
        self._reject_forbidden_inputs(task2_8j_operator_selection, "task2_8j_operator_selection", allow_false_boundary_flags=True)
        ...
        if ot_action_view is not None and not ot_action_view.empty and not affordance.empty:
            ...
            if "graph_object_id" in view_cols:
                affordance = affordance.merge(ot_action_view[view_cols].drop_duplicates("graph_object_id"), on="graph_object_id", how="left")
                affordance["ot_action_view_used"] = True
                affordance["ot_action_view_contract"] = "O_t_action_view_merged_into_action_surface_planning__not_ActionModule_direct_input__Task11_RC1"
        elif not affordance.empty:
            affordance["ot_action_view_used"] = False

        legacy_candidates, sequence_events = self.policy.build_action_frame(pressure_intents, graph_objects, step, seed, scenario)
        ...
        if not action_candidates.empty:
            ...
            action_candidates["action_candidate_contract"] = "ActionSurfacePlanning_pre_ActionFrame_candidate__must_pass_local_audit_and_coactivation_gate__Task11_RC1"
            ...
            action_candidates["exploration_projection_used"] = bool(exploration_projection is not None and not exploration_projection.empty)
            ...
            if ot_action_view is not None and not ot_action_view.empty and "entity_id" in ot_action_view.columns:
                entity_view_cols = [c for c in [
                    "entity_id", "ot_id", "ot_action_relevance_score",
                    "ot_local_observation_need_score", "requires_action_local_audit",
                    "suggested_action_attention", "ot_residual_score", "ot_unresolved_score", "ot_ambiguity_score",
                ] if c in ot_action_view.columns]
                action_candidates = action_candidates.merge(ot_action_view[entity_view_cols].drop_duplicates("entity_id"), on="entity_id", how="left")
                action_candidates["ot_action_view_used"] = True
                action_candidates["ot_action_view_contract"] = "O_t_action_view_used_by_action_surface_planning__not_ActionModule_direct_input__Task11_RC1"
            else:
            ...
            if "task2_8j_primary_candidate" in action_candidates.columns:
                primary_mask = action_candidates["task2_8j_primary_candidate"].fillna(False).astype(bool)
                need.loc[primary_mask] = need.loc[primary_mask].clip(lower=0.35)
        ...
        if not sequence_events.empty:
            ...
            sequence_events["action_surface_planning_contract"] = "sequence_events_from_repaired_policy__side_channel_only__not_ActionModule_direct_input__Task11_RC1"
        ...
        local_observation_needs = self._build_local_observation_needs(action_candidates, ot_action_view, affordance, step, seed, scenario)
    ...
    def _build_task2_8j_primary_candidates(self, *, task2_8j_operator_selection: pd.DataFrame | None, graph_objects: pd.DataFrame, parameter_windows: dict[str, Any], step: int, seed: int, scenario: str) -> pd.DataFrame:
        ...
        for i, (_, r) in enumerate(usable.reset_index(drop=True).iterrows()):
            primitive, channel, role = _operator_to_channel(r.get("risk_name", ""), r.get("selected_operator_name", ""), r.get("selected_operator_family", ""))
    ...
    def _build_local_observation_needs(self, action_candidates: pd.DataFrame, ot_action_view: pd.DataFrame | None, affordance: pd.DataFrame, step: int, seed: int, scenario: str) -> pd.DataFrame:
        if action_candidates is None or action_candidates.empty:
            return pd.DataFrame(columns=["run_seed", "run_scenario", "loop_step", "entity_id", "ot_id", "local_observation_need_score", "requires_action_local_audit", "action_candidate_count", "trigger_reason", "need_source_contract"])
        group = action_candidates.groupby("entity_id", dropna=False).agg(action_candidate_count=("entity_id", "size"), max_action_strength=("action_strength", "max") if "action_strength" in action_candidates.columns else ("entity_id", "size")).reset_index()
        if "ot_local_observation_need_score" in action_candidates.columns:
        ...
        group["trigger_reason"] = group.apply(lambda r: "ot_need_or_candidate_density" if bool(r["requires_action_local_audit"]) else "routine_action_surface_context", axis=1)
        group["need_source_contract"] = "local_observation_needs_from_O_t_action_view_and_pre_gate_action_candidates__Task11_RC1"
        if "task2_8j_primary_candidate" in action_candidates.columns:
        ...
        first = ["run_seed", "run_scenario", "loop_step", "entity_id", "ot_id", "local_observation_need_score", "requires_action_local_audit", "action_candidate_count", "trigger_reason", "need_source_contract", "task2_8j_primary_need_source", "action_frame_created_by_need_builder", "actionmodule_called_by_need_builder"]
        rest = [c for c in group.columns if c not in first]
        return group[first + rest]

    def _build_planning_audit(self, graph_objects: pd.DataFrame, pressure_intents: pd.DataFrame, shadow_params: dict[str, Any], parameter_windows: dict[str, Any] | None, exploration_projection: pd.DataFrame, ot_action_view: pd.DataFrame | None, affordance: pd.DataFrame, action_candidates: pd.DataFrame, local_observation_needs: pd.DataFrame, sequence_events: pd.DataFrame, task2_8j_operator_selection: pd.DataFrame | None, action_route: str, step: int, seed: int, scenario: str) -> pd.DataFrame:
        ...
        candidates_have_required_contract = bool(candidate_rows > 0 and "action_candidate_contract" in action_candidates.columns and action_candidates["action_candidate_contract"].astype(str).str.contains("pre_ActionFrame_candidate", regex=False).all())
        ...
        audit_status = "pass" if (candidate_rows > 0 and affordance_rows > 0 and need_rows > 0 and candidates_have_required_contract and pressure_intent_used and task2_primary_ok and not action_frame_created and not actionmodule_called and not writeback_performed and not lower_direct_to_actionmodule) else "watch"
        row = {
            "run_seed": seed,
            "run_scenario": scenario,
            "loop_step": step,
            "planning_contract": "action_surface_planning_pre_ActionFrame_only__uses_Ot_action_view_pressure_intents_shadow_and_exploration_projection__Task11_RC1",
            ...
            "ot_action_view_used": bool(ot_action_view is not None and not ot_action_view.empty),
    ...
    def _reject_forbidden_inputs(self, df: pd.DataFrame | None, name: str, allow_false_boundary_flags: bool = False) -> None:
        if df is None or df.empty:
            return
        forbidden: list[str] = []
        for c in df.columns:
            if not any(str(c).startswith(prefix) for prefix in _FORBIDDEN_LOWER_DIRECT_COL_PREFIXES):
                continue
            if allow_false_boundary_flags:
                try:
                    if not bool(df[c].fillna(False).astype(bool).any()):
                        continue
                except Exception:
                    pass
            forbidden.append(str(c))
        if forbidden:
