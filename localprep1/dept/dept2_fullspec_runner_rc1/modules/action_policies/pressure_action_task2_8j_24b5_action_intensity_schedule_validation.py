...
REQUIRED_TRUE = [
    "validation_only", "intensity_schedule_validation", "runtime_policy_view_separated", "validation_scoring_view_separated",
    "fixed_weak_medium_strong_tested", "gradual_ramp_tested", "runtime_uses_only_actionmodule_available_information",
    "v2_used_for_scoring_only", "NO_OP_used_for_scoring_only", "side_effect_proxy_from_runtime_available_information_only",
...
FORBIDDEN_TRUE = [
    ...
    "NO_OP_future_used_for_runtime_adjustment", "validation_score_used_as_runtime_input", "release_rollback_audit_shaped",
    "action_candidate_generated", "concrete_action_generated", "action_effect_prediction_generated",
    "effect_prediction_model_executed", "expected_value_final_judgment_performed", "real_actionmodule_called",
    "axis_executed", "upper_pressure_coupled_now", "canonical_write_performed",
]

RUNTIME_COLUMNS = list(BOUNDARY) + [
    "runtime_policy_id", "source_rollout_id", "scenario_profile", "risk_name", "selected_operator_name", "action_wait_step",
    "schedule_name", "schedule_family", "planned_intensity_start", "planned_intensity_peak", "planned_intensity_final",
    "planned_schedule_steps", "runtime_caution_class", "runtime_side_effect_proxy", "runtime_stop_rule",
    "runtime_adjustment_source", "runtime_policy_status",
]
...
COMPARISON_COLUMNS = list(BOUNDARY) + [
    "comparison_id", "schedule_name", "schedule_family", "risk_name", "row_count", "mean_relative_ev", "median_relative_ev",
    "p05_relative_ev", "min_relative_ev", "positive_ev_rate", "action_beats_no_op_rate", "negative_control_pass_rate",
    "large_loss_rate", "severe_loss_rate", "mean_side_effect_delta", "mean_rollback_margin", "mean_boundary_margin",
    "schedule_objective_class", "schedule_review_reason",
]
...
AUDIT_COLUMNS = list(BOUNDARY) + [
    "audit_id", "schedule_name", "risk_name", "row_count", "mean_relative_ev", "large_loss_rate", "severe_loss_rate",
    "mean_scheduled_side_effect_delta", "runtime_side_effect_proxy_mean", "side_effect_gap", "mean_rollback_margin",
    "mean_boundary_margin", "audit_class", "audit_review_reason",
]
...
def _runtime_side_effect_proxy(risk_name: str, operator_name: str, schedule: dict[str, Any], wait: int) -> float:
    base = {
        ...
        "boundary_fragile": 0.22,
        "oscillation": 0.16,
        "relation_lock": 0.14,
        "reversibility_loss": 0.12,
    }.get(str(risk_name), 0.18)
    op_adj = {
        "pressure_diffusion": 0.06,
        "buffer_injection": 0.05,
        "damping": 0.03,
        "soft_resistance": 0.02,
        "reversibility_support": 0.015,
        "escape_channel": 0.04,
    }.get(str(operator_name), 0.03)
    intensity = float(schedule["peak"])
    wait_adj = 0.015 * max(0, int(wait) - 2)
    return round(float((base + op_adj + wait_adj) * max(0.25, intensity)), 6)
...
def build_fixed_intensity_comparison(scoring: pd.DataFrame, cfg: ActionIntensityScheduleValidationConfig) -> pd.DataFrame:
    ...
    for (sched, risk), g in fixed.groupby(["schedule_name", "risk_name"], sort=True):
        ...
        if mean_ev >= cfg.min_mean_ev_for_candidate and large <= cfg.max_large_loss_rate_for_candidate and beat >= 0.60:
            cls = "fixed_intensity_candidate"
            reason = "mean_ev_and_large_loss_constraint_pass"
        elif mean_ev >= 0.03 and large <= cfg.max_large_loss_rate_for_review:
            cls = "fixed_intensity_review"
            reason = "positive_but_tail_or_beat_rate_requires_review"
        else:
            cls = "fixed_intensity_reject"
            reason = "weak_or_large_loss_tail"
        rows.append(_with_boundary({
            "comparison_id": f"fixed_{sched}_{risk}",
            "schedule_name": str(sched),
            "schedule_family": "fixed",
            "risk_name": str(risk),
            "row_count": int(len(g)),
            ...
            "positive_ev_rate": round(float((ev > 0.0).mean()) if len(ev) else 0.0, 6),
            "action_beats_no_op_rate": round(beat, 6),
            "negative_control_pass_rate": round(float(g["scheduled_action_beats_negative_control"].astype(bool).mean()) if len(g) else 0.0, 6),
            "large_loss_rate": round(large, 6),
            "severe_loss_rate": round(severe, 6),
            ...
            "schedule_objective_class": cls,
            "schedule_review_reason": reason,
        }))
...
def build_gradual_ramp_validation(scoring: pd.DataFrame) -> pd.DataFrame:
    ...
    for (sched, risk), g in gradual.groupby(["schedule_name", "risk_name"], sort=True):
        ...
        if mean_ev >= medium_ev and large <= fm_large:
            cls = "gradual_dominates_fixed_medium"
            reason = "higher_or_equal_ev_and_no_worse_large_loss"
        elif large < fm_large and mean_ev >= (medium_ev - 0.04):
            cls = "gradual_safer_tradeoff_candidate"
            reason = "tail_risk_lower_with_small_ev_cost"
        else:
            cls = "gradual_review"
            reason = "does_not_dominate_fixed_medium"
        rows.append(_with_boundary({
            "gradual_id": f"gradual_{sched}_{risk}",
            "schedule_name": str(sched),
            "risk_name": str(risk),
            "row_count": int(len(g)),
            "mean_relative_ev": round(mean_ev, 6),
            "fixed_medium_mean_ev": round(medium_ev, 6),
            "delta_vs_fixed_medium": round(mean_ev - medium_ev, 6),
            "large_loss_rate": round(large, 6),
            "fixed_medium_large_loss_rate": round(fm_large, 6),
            "large_loss_delta_vs_fixed_medium": round(large - fm_large, 6),
            "mean_runtime_side_effect_proxy": 0.0,
            "gradual_schedule_class": cls,
            "gradual_review_reason": reason,
        }))
...
def build_side_effect_large_loss_audit(scoring: pd.DataFrame, runtime: pd.DataFrame) -> pd.DataFrame:
    ...
    for (sched, risk), g in joined.groupby(["schedule_name", "risk_name"], sort=True):
        ...
        if large <= 0.03 and severe <= 0.01 and side_gap <= 0.05:
            cls = "side_effect_and_large_loss_clear"
            reason = "tail_and_side_effect_gap_small"
        elif large <= 0.08:
            cls = "side_effect_or_tail_review"
            reason = "tail_or_runtime_proxy_gap_requires_guard"
        else:
            cls = "side_effect_or_large_loss_reject"
            reason = "large_loss_tail_too_high"
        rows.append(_with_boundary({
            "audit_id": f"side_large_{sched}_{risk}",
            "schedule_name": str(sched),
            "risk_name": str(risk),
            "row_count": int(len(g)),
            ...
            "large_loss_rate": round(large, 6),
            "severe_loss_rate": round(severe, 6),
            "mean_scheduled_side_effect_delta": round(float(side.mean()) if len(side) else 0.0, 6),
            "runtime_side_effect_proxy_mean": round(float(proxy.mean()) if len(proxy) else 0.0, 6),
            "side_effect_gap": round(side_gap, 6),
            "mean_rollback_margin": round(float(g["scheduled_rollback_margin"].astype(float).mean()) if len(g) else 0.0, 6),
            "mean_boundary_margin": round(float(g["scheduled_boundary_margin"].astype(float).mean()) if len(g) else 0.0, 6),
            "audit_class": cls,
            "audit_review_reason": reason,
        }))
...
def build_checks(runtime: pd.DataFrame, scoring: pd.DataFrame, comparison: pd.DataFrame, gradual: pd.DataFrame, audit: pd.DataFrame, source_ready: bool) -> pd.DataFrame:
    ...
    runtime_cols = "|".join(runtime.columns.astype(str).tolist()).lower() if runtime is not None else ""
    checks = [
        ...
        ("check_runtime_no_scoring_columns", "separation", "Runtime columns do not contain scoring/outcome fields.", True, not any(term in runtime_cols for term in runtime_forbidden_terms)),
...
def build_final_summary(source_policy_count: int, runtime: pd.DataFrame, scoring: pd.DataFrame, comparison: pd.DataFrame, gradual: pd.DataFrame, audit: pd.DataFrame, checks: pd.DataFrame, source_ready: bool) -> pd.DataFrame:
    ...
    return pd.DataFrame([_with_boundary({
        ...
        "source_policy_rollout_count": int(source_policy_count),
        "runtime_policy_row_count": int(len(runtime)),
        "scoring_row_count": int(len(scoring)),
        "comparison_row_count": int(len(comparison)),
        "gradual_validation_row_count": int(len(gradual)),
        "side_effect_audit_row_count": int(len(audit)),
        "best_schedule_name": best_name,
        ...
        "gradual_best_mean_ev": round(gradual_best, 6),
        "gradual_best_large_loss_rate": round(gradual_large, 6),
        "resource_pressure_best_schedule": res_best,
        "resource_pressure_best_large_loss_rate": round(res_large, 6),
        ...
        "action_intensity_schedule_validation_decision": decision,
...
def validate_tables(runtime: pd.DataFrame, scoring: pd.DataFrame, comparison: pd.DataFrame, gradual: pd.DataFrame, audit: pd.DataFrame, checks: pd.DataFrame, final_summary: pd.DataFrame) -> list[str]:
    ...
    for name, (table, cols) in tables.items():
        if table is None or table.empty:
            errors.append(f"task2_8j_24b5_empty_table:{name}"); continue
        missing = [c for c in cols if c not in table.columns]
        if missing:
            errors.append(f"task2_8j_24b5_missing_columns:{name}:" + ",".join(missing)); continue
