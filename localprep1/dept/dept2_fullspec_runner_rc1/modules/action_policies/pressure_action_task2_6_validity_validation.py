...
This module validates structural behavior of
...
TASK2_6_VALIDITY_CONTRACT = (
    "Task2_6_validity_validation__converter_structure_only__"
...
def _validate_pair_case(pair: pd.Series, state_case: dict, correspondence: pd.DataFrame) -> dict:
    ...
    status = "pass" if all([all_strengths, no_boost, gap, monotonic_pass, direction_semantics, audit_present]) else "fail"
    notes = []
    if not audit_active:
    ...
    if not notes:
        notes.append("converter invariants passed")
...
def validate_pressure_action_converter_validity_validation_table(df: pd.DataFrame) -> list[str]:
    ...
    for col in ["min_pressure_intent_coverage_score", "min_action_vocabulary_fit_score"]:
