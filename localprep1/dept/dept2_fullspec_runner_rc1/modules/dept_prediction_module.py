...
values only: current values, projected no-action values, deltas,
uncertainty/context signals, and source lineage.
"""
...
class DEPTPredictionModule:
    ...
    def build_relation_projection(self, *, world_trace_before, baseline_trace_after, loop_step: int, seed: int, scenario: str) -> pd.DataFrame:
        columns = ["loop_step", "run_seed", "run_scenario", "source", "target", "world_t_current", "world_t_projection", "relation_metric_name", "current_value", "projected_no_action_value", "projected_no_action_delta", "projection_source", "projection_horizon_steps", "source_trace_fingerprint_current", "source_trace_fingerprint_projection"]
    ...
    def build_global_prediction_summary(self, *, world_trace_before, baseline_trace_after, gt, kt, ot_native, ot_action_view, residual_noise_log, entity_projection, relation_projection, ot_context, loop_step: int, seed: int, scenario: str) -> pd.DataFrame:
        ...
        row = {"loop_step": int(loop_step), "run_seed": int(seed), "run_scenario": str(scenario), "prediction_module_name": self.name, "prediction_build_status": "pass", "source_trace_fingerprint_current": trace_fingerprint(world_trace_before) if world_trace_before else "missing", "source_trace_fingerprint_projection": trace_fingerprint(baseline_trace_after) if baseline_trace_after else "missing", "world_t_current": _first_world_t(world_trace_before), "world_t_projection": _first_world_t(baseline_trace_after), "entity_rows_current": int(len(entity)), "entity_rows_projection": int(len(baseline_entity)), "relation_rows_current": int(len(relation)), "relation_rows_projection": int(len(baseline_relation)), "gt_rows": int(len(gt)) if gt is not None else 0, "kt_rows": int(len(kt)) if kt is not None else 0, "ot_native_rows": int(len(ot_native)) if ot_native is not None else 0, "ot_action_view_rows": int(len(ot_action_view)) if ot_action_view is not None else 0, "residual_noise_log_rows": int(len(residual_noise_log)) if residual_noise_log is not None else 0, "entity_projection_rows": int(len(entity_projection)), "relation_projection_rows": int(len(relation_projection)), "ot_context_rows": int(len(ot_context)), "mean_entity_projected_abs_delta": float(entity_projection["projected_no_action_delta"].abs().mean()) if not entity_projection.empty else 0.0, "mean_relation_projected_abs_delta": float(relation_projection["projected_no_action_delta"].abs().mean()) if not relation_projection.empty else 0.0, "mean_observation_uncertainty": self._mean_ot_metric(ot_context, "uncertainty"), "mean_residual_score": self._mean_ot_metric(ot_context, "ot_residual_score"), "mean_unresolved_score": self._mean_ot_metric(ot_context, "ot_unresolved_score"), "mean_ambiguity_score": self._mean_ot_metric(ot_context, "ot_ambiguity_score"), "mean_macro_micro_mismatch_score": self._mean_ot_metric(ot_context, "ot_macro_micro_mismatch_score"), "unmapped_information_json": _json_dict(unmapped)}
    ...
    def build_prediction_output_packet(self, global_summary: pd.DataFrame, entity_projection: pd.DataFrame, relation_projection: pd.DataFrame, ot_context: pd.DataFrame) -> pd.DataFrame:
        if global_summary is None or global_summary.empty:
            return pd.DataFrame()
        g = global_summary.iloc[0]
        row = {"loop_step": int(g.get("loop_step", -1)), "run_seed": int(g.get("run_seed", -1)), "run_scenario": str(g.get("run_scenario", "unknown")), "prediction_packet_id": f"dept_prediction_packet_s{int(g.get('run_seed', -1))}_t{int(g.get('loop_step', -1))}", "source_trace_fingerprint_current": str(g.get("source_trace_fingerprint_current", "missing")), "source_trace_fingerprint_projection": str(g.get("source_trace_fingerprint_projection", "missing")), "world_t_current": int(g.get("world_t_current", -1)), "world_t_projection": int(g.get("world_t_projection", -1)), "entity_projection_rows": int(len(entity_projection)), "relation_projection_rows": int(len(relation_projection)), "ot_context_rows": int(len(ot_context)), "mean_entity_projected_abs_delta": float(g.get("mean_entity_projected_abs_delta", 0.0)), "mean_relation_projected_abs_delta": float(g.get("mean_relation_projected_abs_delta", 0.0)), "mean_observation_uncertainty": float(g.get("mean_observation_uncertainty", 0.0)), "mean_residual_score": float(g.get("mean_residual_score", 0.0)), "mean_unresolved_score": float(g.get("mean_unresolved_score", 0.0)), "mean_ambiguity_score": float(g.get("mean_ambiguity_score", 0.0)), "mean_macro_micro_mismatch_score": float(g.get("mean_macro_micro_mismatch_score", 0.0)), "packet_content_type": "prediction_values_only"}
        return pd.DataFrame([row])
