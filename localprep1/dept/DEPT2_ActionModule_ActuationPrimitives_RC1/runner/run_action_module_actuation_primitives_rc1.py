#!/usr/bin/env python3
"""DEPT2 ActionModule Actuation Primitives RC1 runner.

Purpose:
    Validate the revised action module contract:
...
def run_one(seed: int, scenario: str, steps: int) -> Dict[str, pd.DataFrame]:
    ...
    primitive_library = planner.primitive_library()
    ...
    collected: Dict[str, List[pd.DataFrame]] = {k: [] for k in [
        ...
        "planned_action_candidates", "final_gate_audit", "action_module_frames", "closed_loop_metrics", "primitive_library",
    ]}
    ...
    for step in range(steps):
        ...
        metrics = pd.DataFrame([{
            ...
            "primitive_action_rows": int(action_frame["action_primitive"].nunique()) if not action_frame.empty and "action_primitive" in action_frame.columns else 0,
        }])
        ...
        frames = {
            ...
            "action_module_frames": action_frame, "closed_loop_metrics": metrics, "primitive_library": primitive_library,
        }
...
def validate_outputs(outputs: Dict[str, pd.DataFrame]) -> dict:
    ...
    primitive_library = outputs.get("primitive_library", pd.DataFrame())
    ...
    summary = {
        "validation_scope": "action_module_actuation_primitives__environment_specific_pseudo_reality__not_performance_proof",
        ...
        "primitive_library_rows": int(primitive_library.drop_duplicates("primitive_id").shape[0]) if not primitive_library.empty and "primitive_id" in primitive_library.columns else 0,
        "planned_unique_primitives": int(planned["action_primitive"].nunique()) if not planned.empty and "action_primitive" in planned.columns else 0,
        "planned_unique_sequences": int(planned["primitive_sequence"].nunique()) if not planned.empty and "primitive_sequence" in planned.columns else 0,
    ...
    summary["all_sanity_checks_passed"] = bool(
        ...
        and summary["primitive_library_rows"] >= 6
        and summary["planned_unique_primitives"] >= 4
